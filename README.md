<!-- Banner (You can design and upload a banner image to your repo, then link it below) -->
<p align="center">
  <img src="https://github.com/SharmAnshu5/Resumes/blob/main/Anshu%20Sharma%20(2).png" alt="Anshu Sharma Banner" width="100%" />
</p>

# 👋 Hi, I'm Anshu Sharma

🎓 Final Year Computer Science & AI/ML Engineering Student  
💻 Backend Developer | AI/ML Enthusiast | Open Source Contributor  
🧠 Passionate about Generative AI, Healthcare AI & Intelligent Systems  
🌟 Building tools that solve real-world problems through code and creativity

---

## ⚙️ Tech Stack

- **Languages**: Python, JavaScript, C++, SQL  
- **AI/ML**: Scikit-Learn, XGBoost, NLTK, SHAP, T5, OpenAI API  
- **Backend**: FastAPI, Flask, Node.js  
- **Frontend**: React.js, Streamlit, Tailwind CSS  
- **Database**: MySQL, SQLite, Firebase  
- **Tools**: Git, Docker, Postman, VS Code

---

## 🚧 Projects

### 🔬 [InsuScan Pro](https://github.com/SharmAnshu5/InsuScan-Pro)
AI-powered medical assistant that:
- Extracts and summarizes diabetic reports  
- Predicts diabetes with high accuracy  
- Tracks patient history and health trends visually  

### 💬 [AI-Powered Chatbot](https://github.com/SharmAnshu5/AI-Powered-Chatbot)
Modern React + Flask chatbot with:
- JWT authentication  
- Smart message UI  
- OpenAI integration for intelligent responses  

### 🌐 [Portfolio Website](https://github.com/SharmAnshu5/Portfolio)
Personal developer portfolio featuring:
- Clean, minimalistic design  
- Project showcases  
- Contact section  

---

## 🚀 Goals & Growth

- 🧠 Expanding expertise in Generative AI and LLMs  
- 🔍 Contributing to impactful AI-driven open-source projects  
- 📊 Making healthcare more accessible with smart tech  

---

## 📬 Get in Touch

- 📧 Email: [anshusharma.cs.ai@gmail.com](mailto:anshusharma.cs.ai@gmail.com)  
- 💼 LinkedIn: [linkedin.com/in/sharmanshu](https://linkedin.com/in/sharmanshu)  
- 🌐 Portfolio: [anshusharma.dev](https://sharmanshu5.github.io/Portfolio/) *(coming soon!)*  

---

## 🏆 GitHub Achievements

![Anshu's GitHub Stats](https://github-readme-stats.vercel.app/api?username=SharmAnshu5&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=SharmAnshu5&layout=compact&theme=tokyonight)

![GitHub Trophies](https://github-profile-trophy.vercel.app/?username=SharmAnshu5&theme=tokyonight&row=1&column=6)

---

## 💡 Quote

> *"Code is the tool. Curiosity is the engine. Progress is the journey."*  
> — Anshu Sharma



